...
function getDayTransitionValue(tick) {
...
PlayerInterval.subscribe(player => {
    ...
    if ((playerData.accessory_slot.relic_slot == "miki:relic_of_eternal_night") && (playerData.flag.relic_of_eternal_night_day))
        lethal_target = true;
    if (lethal_target && !player.hasTag("miki:lethal_immunity"))
        player.addTag("miki:lethal_immunity");
    else if (!lethal_target && player.hasTag("miki:lethal_immunity"))
        player.removeTag("miki:lethal_immunity");
});
    ...
    switch (signal.id) {
        case "miki:lethal_target":
            /*
                玩家觸發免死
            */
            if (signal.sourceEntity === undefined || signal.sourceEntity.typeId != "minecraft:player")
                return;
