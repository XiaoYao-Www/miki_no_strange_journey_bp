...
import { ActionFormStructure, ButtonStructure, DropdownStructure, FormStructure, FormStructureType, FormWidgetType, MessageFormStructure, ModalFormStructure, SliderStructure, TextFieldStructure, ToggleStructure } from "../classes/uflib_form_class.js";
...
export class UFLib_Form {
     ...
     * 玩家路徑紀錄
     */
    private static _playerFormStacks: Map<string, FormStructure[]> = new Map();
     ...
     */
    static show(player: Player, formStructure: FormStructure): void {
        try{
            // 處理生成器
     ...
     * @param formStructure 表單結構
     */
    static open(player: Player, formStructure: FormStructure): void {
        try {
            const playerFormStack: FormStructure[] = this._playerFormStacks.get(player.id) ?? [];
     ...
     * @param formStructure 表單結構 
     */
    static replace(player: Player, formStructure: FormStructure): void {
        try {
            const playerFormStack: FormStructure[] = this._playerFormStacks.get(player.id) ?? [];
            if (playerFormStack.length > 0) playerFormStack.pop();
            playerFormStack.push(formStructure);
            this._playerFormStacks.set(player.id, playerFormStack);
            this.show(player, formStructure);
        } catch (error: any) {
     ...
     * @param count 回退層數
     */
    static back(player: Player, count: number = 1): void {
        try {
            if (count < 1) return;
            const playerFormStack: FormStructure[] = this._playerFormStacks.get(player.id) ?? [];
            if (playerFormStack.length > count) {
                for (let i = 0; i < count; i++) playerFormStack.pop();
                this._playerFormStacks.set(player.id, playerFormStack);
     ...
     * @param player 玩家
     */
    static clearStack(player: Player): void {
        try {
     ...
     * @param formStructure 表單結構
     */
    private static showActionForm(player: Player, formStructure: ActionFormStructure): void {
        try {
            // 創建表單
            ...
            form.show(player).then((response: ActionFormResponse) => {
                if (response.canceled && formStructure.canceledAction !== undefined) {
                    ...
                    // 選項定義回傳行為
                    const button: ButtonStructure | undefined = formStructure.buttons[response.selection];
                    if (button?.action !== undefined) {
                        button.action(player);
                    }
                    // 表單定義回傳行為
                    if (formStructure.action !== undefined) formStructure.action(player, response.selection);
                }
            }).catch((error: any) => {
                throw new UFLib_Form.FunError("error", { cause: error });
            });
     ...
     * @param formStructure 表單結構
     */
    private static showModalForm(player: Player, formStructure: ModalFormStructure): void {
        try {
            // 創建表單
                ...
                if (response.canceled && formStructure.canceledAction !== undefined) {
                    ...
                    if (response.formValues.length == formStructure.widgets.length) {
                        for (let index = 0; index < response.formValues.length; index++) {
                            ...
                            switch(widget.type){
                                case FormWidgetType.dropdown:
                                    widget.action?.(player, response.formValues[index] as number);
                                    break;
                                ...
                                case FormWidgetType.textField:
                                    widget.action?.(player, response.formValues[index] as string);
                                    break;

                                case FormWidgetType.toggle:
                                    widget.action?.(player, response.formValues[index] as boolean);
                                    break;
                    ...
                    // 表單定義回傳行為
                    if (formStructure.action !== undefined) formStructure.action(player, response.formValues);
                }
            }).catch((error: any) => {
                throw new UFLib_Form.FunError("error", { cause: error });
            });
     ...
     * @param formStructure 表單結構
     */
    private static showMessageForm(player: Player, formStructure: MessageFormStructure): void {
        try {
            // 創建表單
